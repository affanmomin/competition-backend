...
// ----------------- CONFIG -----------------
const username = "adnanchanda006@gmail.com";
const password = "Testdev@123";
...
const SEL_POST_TEXT_MAIN =
  '.update-components-update-v2__commentary .break-words [dir="ltr"]';
...
const SEL_SOCIAL_COUNTS =
  ".update-v2-social-activity .social-details-social-counts";
...
async function humanPause(base = 300, variability = 250) {
  await sleep(base + rand(0, variability));
}
...
async function humanScrollTo(page: Page, y: number) {
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    await page.evaluate(
      (val) => window.scrollTo({ top: val, behavior: "instant" }),
      yy,
    );
...
async function makeStealthyContext(
  browser: Browser,
...
async function openCommentsInCard(card: ElementHandle<Element>, page: Page) {
  ...
  if (!btn) {
    btn = await card.$(
      `${SEL_SOCIAL_COUNTS} [aria-label*="comment" i], ${SEL_SOCIAL_COUNTS} button[aria-label*="comment" i]`,
    );
  }
  if (btn) {
...
async function loadMoreCommentsInCard(
  ...
  for (let i = 0; i < MAX_COMMENT_PAGES; i++) {
    ...
    const count = await card.evaluate(
      (root, sel) => root.querySelectorAll(sel).length,
      SEL_COMMENT_ITEMS,
    );
  ...
  return await card.evaluate(
    function (root, cfg) {
      function getInner(el) {
        return el && "innerText" in el ? (el.innerText || "").trim() : "";
      }
      ...
      function parseCount(keyword, scope) {
        var elList = (scope || root).querySelectorAll(
          "[aria-label], button, span, div",
        );
        for (var i = 0; i < elList.length; i++) {
          var aria =
            elList[i].getAttribute && elList[i].getAttribute("aria-label");
      ...
      function detectMedia(base, selImg) {
        ...
        if (img && img.getAttribute("src"))
          return {
            mediaType: "Image",
            mediaLink: img.getAttribute("src") || "",
          };
      ...
      // TEXT (scoped to root)
      var text = pickFirstInner(
        root,
        [cfg.SEL_POST_TEXT_MAIN].concat(cfg.SEL_POST_TEXT_FALLBACKS),
      );
...
function convertAbbreviatedToNumber(s: string): number {
  if (!s) return 0;
  const numPart = (s.match(/[\d,.]+(?:\s*[KM])?/i) || [""])[0].replace(
    /[,\u00A0]/g,
    "",
  );
...
export async function scrapeCompanyPosts(companyName: string): Promise<any[]> {
  const { pageUrl, companyPostUrl, companyNameTitle } =
    generateCompanyNames(companyName);
  ...
  const context = await makeStealthyContext(
    browser,
    fs.existsSync(STORAGE_STATE_PATH) ? STORAGE_STATE_PATH : undefined,
  );
  ...
  try {
    ...
    if (!hasSavedState) {
      ...
      if (userEl && passEl) {
        ...
        if (submit) await humanClickElement(page, submit);
      ...
      const loggedInIndicator = await page.$(
        "header, nav, #profile-nav-item, .global-nav__me-photo, .nav-item__profile-member-photo",
      );
      if (loggedInIndicator) {
        ...
        if (hasLiAt) {
          ...
          if (DEBUG)
            console.log(
              "Saved storageState (cookie-based) to",
              STORAGE_STATE_PATH,
            );
        } else {
          console.warn(
            "Login probably failed or required MFA. No storageState saved.",
          );
        }
      }
    } else if (DEBUG) {
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      try {
        ...
        if (INCLUDE_COMMENTS) {
          ...
          try {
            await card.waitForSelector(SEL_COMMENTS_CONTAINER, {
              timeout: 2000,
            });
          } catch {}
...
async function run() {
  ...
  try {
    ...
    // Export results
    await exportCsv(
      `${companyNameTitle}_posts.csv`,
      results.map(({ comments, ...flat }) => flat),
    );
    await fs.writeJson(
      `${companyNameTitle}_posts_with_comments.json`,
      results,
      { spaces: 2 },
    );
