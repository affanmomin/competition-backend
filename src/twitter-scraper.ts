...
function generateTwitterNames(profileHandle: string) {
  ...
  const profileNameTitle = profile_slug.replace(/\b\w/g, (c) =>
    c.toUpperCase(),
  );
...
async function humanWheelScroll(page: Page, range: [number, number] = [3, 6]) {
  ...
  for (let i = 0; i < reps; i++) {
    ...
    const deltaBase = Math.floor(
      (await page.evaluate(() => window.innerHeight)) *
        (0.45 + HUMAN_PROFILE.scrollAggro * 0.6),
    );
...
async function humanType(page: Page, selector: string, text: string) {
  ...
  for (const ch of chars) {
    ...
    await page.keyboard.type(ch, {
      delay: rand(60, 150) * HUMAN_PROFILE.paceMultiplier,
    });
...
async function collectTweetUrls(
  page: Page,
  limit: number,
): Promise<{ url: string; likes: string }[]> {
          ...
          try {
            ...
            if (likeAction) {
              const countText = await likeAction.evaluate((node) => {
                const group =
                  node.closest('div[role="group"]') || node.parentElement;
                if (group) {
...
async function scrapeTweetPage(
  ...
  tweetObj: { url: string; likes: string },
  index: number,
) {
  const page = await context.newPage();
  try {
    ...
    try {
      await page.waitForSelector(
        'article[data-testid="tweet"] div[lang], article[role="article"] div[lang]',
        { timeout: 8000 },
      );
      mainTweet = await page.$eval(
        'article[data-testid="tweet"] div[lang], article[role="article"] div[lang]',
    ...
    while (
      commentMap.size < MAX_COMMENTS_PER_POST &&
      ...
      for (let i = 1; i < Math.min(articles.length, 40); i++) {
        try {
          const username = await articles[i]
            .$eval('div[data-testid="User-Name"] span', (el) =>
              el.innerText.trim(),
            )
            .catch(() => "");
          const commentText = await articles[i]
  ...
  try {
    ...
    for (let i = 0; i < tweetObjs.length; i += PARALLEL_LIMIT) {
      ...
      const batchResults = await Promise.all(
        batch.map((t, j) => scrapeTweetPage(context, t, i + j + 1)),
      );
...
async function run(company: string) {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(dataDir, "tweets_with_comments.json"),
